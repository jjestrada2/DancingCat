...
export default function Home() {
    ...
    const getAnimalDetails = (token, tokenHash, animalId) => {
        ...
        const req = {
            ...
            fields: ["animalID", "animalName", "animalBreed", "animalHighlightOrder", "animalPictures"],
        }
        fetch("https://api.rescuegroups.org/http/v2.json", {
            method: "Post",
            body: JSON.stringify(req)
        }).then(response => response.json(response)).then((result) => {
            setData(result.data[0])
            result.data[0]?.animalPictures.length > 0 ? setAnimalPicture(result.data[0].animalPictures[0].urlInsecureFullsize) : null
        })
    }
